...
export default function EditListingPage() {
  ...
  useEffect(() => {
    ...
    fetch(`${process.env.NEXT_PUBLIC_API_URL || `${process.env.NEXT_PUBLIC_API_URL || 'http://127.0.0.1:8001'}`}/api/listings/${id}`)
      .then(res => res.json())
      .then(data => {
        setFormData({
          title: data.title,
    ...
    <div className="min-h-screen flex flex-col bg-gray-50">
      ...
      <main className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-10 w-full flex-grow">
        <div className="flex justify-between items-center mb-8">
          <h1 className="text-3xl font-bold text-airbnb-text">Edit your place</h1>
          <button onClick={handleDelete} className="text-red-600 font-semibold hover:underline">Delete Listing</button>
        </div>
